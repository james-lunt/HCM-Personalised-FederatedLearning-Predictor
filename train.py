...
def prepare_batch(batch, device):
    ...
    with open(Path(config['paths']['misc']).joinpath(config['names']['labelencoder']), 'rb') as handle: #MESHMESH
    #with open(HOME_PATH / Path(config['paths']['misc']).joinpath(config['names']['labelencoder']), 'rb') as handle:
        le = pickle.load(handle)
    with open(Path(config['paths']['misc']).joinpath('le.pkl'), 'rb') as handle: #MESHMESH
...
if __name__=='__main__':
    ...
    for fold in range(finalfold):
        ...
        train_losses, val_losses, test_predictions, confusion_matrices, test_averages = train(model = model,
            dataset = data,
            opt=opt,
            criterion=criterion,
            device=device,
        ...
        #log_dict['test_accuracies'].append(test_averages)
