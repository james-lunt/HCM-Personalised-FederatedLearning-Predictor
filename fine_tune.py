...
#Prints the result metrics every epoch aand updates early stop counter
def log_results(epoch,train_loss,val_loss,val_accuracy,confusion_matrix, best_model_results,best_model,early_stop_counter):
    ...
    if val_loss < best_model_results['val_loss']:
        ...
        best_model = initialize_model(model.state_dict())
    else:
...
def fine_tune(model,data,opt,criterion,num_epochs):
    ...
    #Accuracy to beat
    #_, _, test_predictions, _, epoch_accuracies, test_confusion_matrix = run_epoch(FOLD,test_loader, model=best_model, opt=opt, criterion=criterion, device=DEVICE, is_training = False)
    ...
    for epoch in range(0, num_epochs):
        ...
        #Log
        best_model_results,best_model,early_stop_counter = log_results(epoch,train_loss_avg,val_loss_avg,val_accuracy_avg,confusion_m, best_model_results,best_model,early_stop_counter)
...
if __name__=='__main__':
    ...
    save_best_model(best_model, best_model_results['epoch'],MODEL_STORAGE)
""" for epoch in range(0,num_epochs):
